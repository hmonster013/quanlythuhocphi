...
--Tìm những môn học của sinh viên A đã đăng ký trong học kỳ B
...
CREATE PROCEDURE sp_XULYDANGKY_select_tenmh(
	@MADK int,
	@MASV nvarchar(15),
	@HOCKY int,
	@TENMH nvarchar(100)
...
BEGIN
	...
	FROM DANGKY INNER JOIN CTDANGKY ON DANGKY.MADK = CTDANGKY.MADK
				...
				INNER JOIN MONHOC ON LOPHOCPHAN.MAMH = MONHOC.MAMH
				INNER JOIN GIANGVIEN ON LOPHOCPHAN.MAGV = GIANGVIEN.MAGV
	WHERE DANGKY.MASV = @MASV AND DANGKY.MADK = CTDANGKY.MADK 
							AND DANGKY.HOCKY = @HOCKY 
							AND MONHOC.TENMH LIKE '%' + @TENMH + '%'
END
go

--Tìm những môn học mà sinh viên chưa đăng ký trong học kỳ A
...
begin
	...
	FROM 
		...
		FROM SINHVIEN INNER JOIN LOP ON SINHVIEN.MALOP = LOP.MALOP
					...
					INNER JOIN MONHOC ON LOPHOCPHAN.MAMH = MONHOC.MAMH
		WHERE SINHVIEN.MASV = @MASV)  as allplh
	WHERE MAMH NOT IN 
		(	
		SELECT MONHOC.MAMH
		FROM DANGKY INNER JOIN CTDANGKY ON DANGKY.MADK = CTDANGKY.MADK
